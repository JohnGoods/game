// TypeScript file
class OpenTXAdvancedWindow extends BaseCtrlWnd {
    ...
    public initObj(...params: any[]) {
        ...
        this.name = params[2] || "txa"                  //天仙进阶
        this.mLayoutPath = params[3]
        this.actIndex = PayActivityIndex.NEW_SERVER_STAGE_LEVEL_UP
        this.timerList = {}
        this.cellIndex = - 1
    }
    ...
    onRefresh() {
        ...
        for (let k in l) {
            let v = l[k]
            if (playerInfo && playerInfo[v.index] == 1) {           //已领取
    ...
    refreshItemWindow(window, config) {
        ...
        if (playerInfo[config.index] != null) {
            if (playerInfo[config.index] == 0) {                  //可领取，未领取
            ...
            } else if (playerInfo[config.index] == 1) {           //已领取
                this.mElemList[name + "_gain"].visible = true
