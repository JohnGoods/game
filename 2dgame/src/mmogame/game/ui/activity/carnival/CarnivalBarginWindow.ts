// TypeScript file
class CarnivalBarginWindow extends BaseCtrlWnd {
    ...
    public initObj(...params: any[]) {
        ...
        this.name = params[2] || "bargin"                  //折扣商店
        this.mLayoutPath = params[3]
        this.activityIndex = PayActivityIndex.SHOP_DISCOUNT_B
        this.cellIndex = -1
    }
    ...
    onRefresh() {
        let tempList = GetActivity(ActivityDefine.Carnival).getCarnivalShopList(this.activityIndex, GameConfig.ShopDiscountBConfig)
        ...
        for (let k = 0; k < 2; k++) {
            ...
            if (config) {
                ...
                if (nowStage < config.cond) {
                    ...
                    this.mElemList[name + "_unlocktl" + k].text = String.format(Localize_cns("CARNIVAL_UNLOCK_XIANWEI_ENOUGH"), Localize_cns(cellOptionsName[funType - 1]), config.cond)
                } else {
    ...
    onClickGo(args) {
        ...
        if (playerInfo[config.index] >= config.limitCount) {
            return
        }
