// TypeScript file
class Wonder_AccumulateWnd extends BaseCtrlWnd {
	name: string;
	mLayoutPath: any;
	...
	public initObj(...params: any[]) {
		...
		this.activityIndex = PayActivityIndex.Recharge_have_prize
	}
	...
	public onLoad(): void {
		...
		let mElemInfo = [
			...
			{ ["name"]: this.name + "recharge_txt", ["title"]: "", ["font"]: "ht_22_rc_stroke_zongse", ["color"]: gui.Color.orange },
			{ ["name"]: this.name + "recharge_btn", ["title"]: Localize_cns("ACTIVITY_PAY_TXT16"), ["font"]: "ht_22_cc_stroke_zongse", ["color"]: gui.Color.white, ["event_name"]: egret.TouchEvent.TOUCH_TAP, ["fun_index"]: this.onClickLink }
		]
		UiUtil.initElem(mElemInfo, this.mElemList[this.name], this.mElemList, this)

		let list: eui.List = this.mElemList[this.name + "scroll"]
		list.itemRenderer = itemRender.WonderAccumulateItem
	}
	...
	onRefresh() {
		...
		let dataConfig = GameConfig.AccRechargeGiftConfig
		let dataList = []
	...
	updateWnd() {
		...
		this.mElemList[this.name + "recharge_txt"].text = String.format(Localize_cns("CARNIVAL_LEIJI_RECHARGE"), GetRmbFromGold(record))
	}
...
module itemRender {
	export class WonderAccumulateItem extends eui.ItemRenderer {
		mElemList: any;
		constructor() {
			super();
			this.mElemList = {}
		...
		protected dataChanged(): void {
			...
			let limit = GetRmbFromGold(this.data.needPoint)
			...
			this.mElemList["title"].text = String.format(Localize_cns("WONDER_ACCUMULATE_TXT1"), limit)
