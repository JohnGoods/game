class ForgeStrengthenWindow extends BaseCtrlWnd {
    mElemList;
    recvList;
    select;
    stage;
    ...
    public onShow(): void {
        ...
        this.mElemList["btn_cancel"].visible = true
        this.mElemList["label_auto"].visible = true
        this.mElemList["label_wndName"].text = Localize_cns("FORGE_TXT1");
        let str = String.format(Localize_cns("FORGE_BTN"), Localize_cns("FORGE_TXT1"))
    ...
    public onHide(): void {
        ...
        this.mElemList["btn_cancel"].visible = false
        this.mElemList["label_auto"].visible = false
        if (this.timer != null) {
    ...
    onRefreshDes() {
        ...
        if (this.timer == null) {
            ...
            let had = GetHeroMoney(costConfig.moneyunit)
            let maxCost = costConfig.money
            ...
            let itemStr = GetMoneyIcon(costConfig.moneyunit)
    ...
    onOneKeyClick() {
        ...
        let index = this.mParentWnd.tabWndList.getTabIndex()
        if (index == 0) {
            //let countId = GameConfig.FunForgeConfig[this.type][this.recvList[this.select-1]+1].moneyunit
            let had = GetHeroProperty("funds") //ItemSystem.getInstance().getItemCount(countId)
            let maxCost = GameConfig.FunForgeConfig[this.type][this.recvList[this.select - 1] + 1].money
            if (had < maxCost) {
                let wnd = WngMrg.getInstance().getWindow("MoneyChargeFrame")
                wnd.showWnd()
            } else {
                this.oldlist = this.recvList
                this.isSend = true
