class PetTongLinWindow extends BaseCtrlWnd {
	mElemList;
	soulType: number;
	soulInfo: any;
	select: number;
	...
	public onLoad(): void {
		...
		this.mElemList["actor_tl_pet"] = UIActorView.newObj(this.mLayoutNode, "actor_tl_pet", 0, 0, this.mElemList["pet_tl_wnd"])
	}
	...
	public onShow(): void {
		...
		this.mElemList["actor_tl_pet"].setVisible(true)
	...
	public onHide(): void {
		...
		this.mElemList["actor_tl_pet"].clearView()
		this.mElemList["actor_tl_pet"].setVisible(false)
	}
	...
	refreshActorWnd() {
		...
		if (info) {
			let petActor = <UIActorView>this.mElemList["actor_tl_pet"]
	...
	onTipsClick() {
		...
		wnd.showWithActivity("tonglingRule")
	}
