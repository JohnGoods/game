class ShopGlobalFrame extends BaseWnd {
    mElemList;
    select: any;
    type
    ...
    updateCostWnd() {
        let groupName = ShopSystem.SHOP_GLOBAL
        let judgeLimit = opJudgeJieSuo.GLOBALNUM

        let hadLimit = ShopSystem.getInstance().getHeroJudge(judgeLimit) || 0
        let limitStr = Localize_cns("SHOP_HAD_TXT10") + hadLimit
