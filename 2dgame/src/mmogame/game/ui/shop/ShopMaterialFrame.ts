class ShopMaterialFrame extends BaseWnd {
    mElemList;
    select: any;
    type
    ...
    updateCostWnd() {
        let groupName = ShopSystem.SHOP_MATERIAL
        let judgeLimit = opJudgeJieSuo.MATNUM

        let hadLimit = ShopSystem.getInstance().getHeroJudge(judgeLimit) || 0
        let limitStr = Localize_cns("SHOP_HAD_TXT12") + hadLimit
