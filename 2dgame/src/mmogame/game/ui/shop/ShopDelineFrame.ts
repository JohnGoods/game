class ShopDelineFrame extends BaseWnd {
    mElemList;
    select: any;
    type
    ...
    updateCostWnd() {
        let judgeLimit = opJudgeJieSuo.DELINENUM
        let groupName = ShopSystem.SHOP_DELINE
        let hadLimit = ShopSystem.getInstance().getHeroJudge(judgeLimit) || 0
        let limitStr = Localize_cns("SHOP_HAD_TXT11") + hadLimit
